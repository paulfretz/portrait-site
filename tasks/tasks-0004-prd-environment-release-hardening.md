...
- `.github/workflows/nightly-orphan-cleanup-prod.yml` - New nightly job to audit orphaned S3 objects in production.
- `.github/workflows/nightly-orphan-cleanup-dev.yml` - New nightly job to audit orphaned S3 objects in development.
- `.github/workflows/nightly-orphan-cleanup-test.yml` - New nightly job to audit orphaned S3 objects in test.
...
- `scripts/orphan-audit.ts` - New script for comparing S3 objects to database references.
...
- [ ] 1.0 Stand up three Supabase + S3 environments (prod, dev, test) with documented credentials and guardrails
  - [ ] 1.1 Provision or confirm Supabase targets for prod, test (hosted) and dev (Supabase CLI); record project IDs/connection info.
  - [ ] 1.2 Provision S3 buckets or prefixes for prod/test/dev; choose naming convention and document in `docs/environment-setup.md`.
  - [ ] 1.3 Update `.env.example`, `.env.local`, and `.env.test` to include AWS credentials, bucket names/prefixes, and instructions.
...
- [ ] 2.0 Migrate media storage to S3 and implement nightly orphan-audit workflows
  - [ ] 2.1 Build a reusable audit script (`scripts/orphan-audit.ts`) to list S3 objects (by prefix) and compare against DB image URLs.
  ...
  - [ ] 2.4 Update upload API route(s) to generate presigned S3 URLs and adjust admin UI uploader to use them.
  - [ ] 2.5 Front S3 with a CDN (CloudFront or Cloudflare), configure cache rules, and document the CDN endpoint.
  - [ ] 2.6 Add optional delete mode or retention window logic to the audit script; leave default as dry-run.
  - [ ] 2.7 Configure notifications (email/Slack/etc.) for orphan findings and document how to review/deal with results.
- [ ] 3.0 Update CI/CD to enforce test-only databases/storage for automated suites and gate migrations
  - [ ] 3.1 Update `.github/workflows/ci.yml` and `.github/workflows/playwright.yml` so every job uses `TEST_SUPABASE_*` and `TEST_S3_*` variables.
...
- [ ] 4.0 Complete deferred launch-readiness checklist (audits, device QA, deployment docs, analytics)
  ...
  - [ ] 4.4 Set up production Vercel env vars, Supabase prod configuration, S3/CDN credentials, email service, and confirm they match documentation.
